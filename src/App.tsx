...
const DetailFunction: React.FC<DetailProps> = (props) => {
  const onNumOfPeopleChange = (
...
const SummaryFunction: React.FC<SummaryProps> = (props) => {
  return (
    <div>
      <div className="party">
...
> {
  constructor(props: {}) {
    ...
    const infans: FeeClassification = {
      name: '幼児',
      description: '未就学',
      unitPrice: 0,
      numOfPeople: 0,
    ...
    this.state = {
      feeClassifications: [adults, students, children, infans],
    }
  }
  ...
  render() {
    const details = this.state.feeClassifications.map((fc, idx) => {
      return (
        <DetailFunction
          key={idx.toString()}
          classification={fc}
          onNumOfPeopleChange={(n: any) => this.handleNumOfPeopleChange(idx, n)}
        />
      )
    })
    ...
    return (
      <>
        {details}
        <SummaryFunction numOfPeople={numOfPeople} totalAmount={totalAmount} />
      </>
    )
  }
}
